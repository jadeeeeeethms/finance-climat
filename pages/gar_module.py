...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame: 
    ...
    rename_map = {"Company Name": "Company name","Nace Code": "NACE Code","Portfolio Weight %": "Portfolio Weight (%)","Portfolio weight (%)": "Portfolio Weight (%)","Taxonomy eligibility": "Taxonomy Eligibility","EU Taxonomy alignment (%)": "EU Taxonomy alignment","EU taxonomy alignment": "EU Taxonomy alignment",}
...
def run_gar_module(df: pd.DataFrame):
    ...
    if df.empty:
        ...
        return
    df["Is_Financial"] = df["NACE Code"].str.upper().str.startswith("K") #Exclude financial sector K from denominator
    ...
    cap_limit = st.sidebar.slider( "Max Weight Cap per Issuer (%)", min_value=5, max_value=30, value=15, key="gar_cap") / 100.0

    current_gar = np.sum( df_nfc["Normalized_Weight"] * (df_nfc["EU Taxonomy alignment"] / 100.0))
    ...
    with col2:
        st.metric( "Optimized GAR", f"{optimized_gar * 100:.2f}%", delta=f"{(optimized_gar - current_gar) * 100:.2f} pp")

    with col3:
        dqs_score = (df_nfc.loc[df_nfc["Data_Source"] == "Reported", "Normalized_Weight"].sum() * 100 )
    ...
    df_chart = df_nfc.melt( id_vars=["Company name", "EU Taxonomy alignment"], value_vars=["Normalized_Weight", "Optimized_Weight"], var_name="Scenario", value_name="Weight")

    fig_bar = px.bar(df_chart,x="Company name",y="Weight",color="Scenario",barmode="group",hover_data=["EU Taxonomy alignment"])
    ...
    with st.expander("Debug GAR input"): #debug pr voir
